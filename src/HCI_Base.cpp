...
/*
 * HCI_CMDPkg_Gen:: HCI command package
 *
...
int HCI_Base::HCI_CMDPkg_Gen(uchar* buffer, int buffMaxL, uint16_t OPCode,
	...
	if (params != buffer + HCI_CMDPkg_Gen_PADDING)
		memcpy(buffer + HCI_CMDPkg_Gen_PADDING, params, paramL);

	uint16_t* OPSection = (uint16_t*) buffer;//quite messy, would change later
	*(OPSection)++ = OPCode;
	buffer = (uchar*) OPSection;
	*buffer++ = paramL;
 ...
 *
 */
int HCI_Base::HCI_SLIP_ENC(uchar* buffer, int buffMaxL, uchar* rawPkg,
		int rawPkgL, bool OOFSWFlowCtrl) {
	int extendN = 0;

	for (int i = 0; i < rawPkgL; i++)//find how much uchar need to be replaced
	{
		uchar tmp = rawPkg[i];
		if (tmp == 0xC0 || tmp == 0xDB)
			extendN++;
		else if (OOFSWFlowCtrl && (tmp == 0x11 || tmp == 0x13))
			extendN++;
	}
...
/*
 *
 *   |header(4B)|payload(xB)|CRC_CCITT(2B)|
 *   |           \__________________________________________________________________________________________
 *   |                                 header(4B=32b)                                                       |
 *   |seqNum(3b)| ackNum (3b)| dataIntegrity(1b)| reliablePkg(1b)| pkgType(4b)| payloadL(16b)| check sum(8b)|
 *
 */

int HCI_Base::HCI_3WUART_PK(uchar* buffer, int buffMaxL, uchar seqNum,
		uchar ackNum, bool dataIntegrity, bool reliablePkg, uchar pkgType,
		uchar* payload, int payloadL) {

	Header_3WireUart header;
	header.elements.seqNum = seqNum;
	header.elements.ackNum = ackNum;
	header.elements.dataIntegrity = dataIntegrity;
	header.elements.reliablePkg = reliablePkg;
	header.elements.pkgType = pkgType;
	header.elements.payloadL = payloadL;
	header.elements.headerChecksum = 0xFF ^ header._4bytes[0]
			^ header._4bytes[1] ^ header._4bytes[2];

	return HCI_3WUART_PK(buffer, buffMaxL, header.num, payload, payloadL);
}
int HCI_Base::HCI_3WUART_PK(uchar* buffer, int buffMaxL,
		Header_3WireUart header, uchar* payload, int payloadL) {
	header.elements.payloadL = payloadL;
	header.elements.headerChecksum = 0xFF ^ header._4bytes[0]
			^ header._4bytes[1] ^ header._4bytes[2];
	return HCI_3WUART_PK(buffer, buffMaxL, header.num, payload, payloadL);
}
int HCI_Base::HCI_3WUART_PK(uchar* buffer, int buffMaxL, uint32_t headerCode,
		uchar* payload, int payloadL) {

	Header_3WireUart header;
	header.num = headerCode;
	if (buffMaxL < (payloadL + HCI_3WUART_HEADERSIZE
			+ ((header.elements.dataIntegrity) ? 2 : 0)))
		return -1;//check space

	if (payload != buffer + HCI_3WUART_HEADERSIZE)//if it in the same position, don't need copy (short cut)
		memcpy(buffer + HCI_3WUART_HEADERSIZE, payload, payloadL);//fill payload


	*((uint32_t*) buffer) = headerCode;//assign header

	if (!header.elements.dataIntegrity)
		return payloadL + HCI_3WUART_HEADERSIZE;
	...
	*((uint16_t*) (buffer + payloadL + HCI_3WUART_HEADERSIZE))
	...
	//Set 16 bit CRC_CCITT to position [payloadL+4]
	//may choose other CRC resources to generate it
...
uint16_t HCI_Base::Calculate_CRC_CCITT(const uchar* buffer, int size) {
	uint16_t tmp;
	uint16_t crc = 0xffff;
